...
export function StaffRequestDetailPage() {
  ...
  const requestQuery = useStaffRequest(id ?? "");

  const assignMutation = useAssignStaffRequest(id ?? "");

  const statusMutation = useUpdateRequestStatus(id ?? "");

  const approvalsQuery = useApprovals(id ?? "");
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div>
          <p className="text-sm text-gray-500">Staff / Requests / Detail</p>
        ...
        <div className="flex gap-2">
          <span
            className={`rounded-full px-3 py-1 text-sm font-medium ${statusClasses[request.status]}`}
          >
